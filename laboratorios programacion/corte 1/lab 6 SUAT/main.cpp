#include <iostream>
// para poner a correr el codigo primero vamos a dar ctrol + shif + b, esto me lo compilara
// para ejecutarlo le damos ctrol shit ñ, y a continuacion escribimos dir, esperamos a que
// carge. luego le damos ".\main.exe"

// ahora vamos a configurar el debuggin. le damos en run y en star debuggin (f5),  elegimos gdb, luego seleccionamos la
// primera opcion que dice c++
...
public:
    ...
    string siAprobo(float prom)
    {
        ...
        if (prom >= 3.0)
        {
            res = "---aprobo la asignatura---";
            cout << endl;
        }
        else
        {
            res = "---no aprobo la asignatura---";
            cout << endl;
        }
        return res;
    }
    ...
    while (opc != 5)
    {
        ...
        if (opc > 0 && opc < 5)
        {
            if (opc == 1)
            {
                ...
                {
                    cout << "no se pueden ingresar mas estudiantes";
                    cout << endl
                         << endl;
                    system("pause");
                }
            ...
            {
                mostrarTodosEstudiantes(e1, e2, e3);
                system("pause");
            }
            else if (opc == 4)
            {
                int elim;
                cout << "que estudiante desea eliminar: ";
                cin >> elim;
                if (elim == 1)
                {
                    e1.resetear();
